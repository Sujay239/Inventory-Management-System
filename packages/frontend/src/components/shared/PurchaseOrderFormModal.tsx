...
import { SupplierFormData } from "./SupplierFormModal";
...
interface PurchaseOrderFormModalProps {
  ...
  existingOrders: PurchaseOrderFormData[];
...
const generateUniqueRef = (existingOrders: PurchaseOrderFormData[]) => {
  let newRef = "";
  let isUnique = false;
  while (!isUnique) {
    newRef = `PO-${Math.floor(Math.random() * 100000)
      .toString()
      .padStart(5, "0")}`;
...
export function PurchaseOrderFormModal({
  ...
  const [formData, setFormData] =
    useState<PurchaseOrderFormData>(defaultFormData);
    ...
    setFormData((prev) => {
      const newItems = prev.items.map((item) => {
        if (item.id === rowId) {
          ...
          // If product changes, auto-fill its cost price
          if (field === "productId") {
            const product = products.find((p) => p.id === value);
            if (product) {
              updatedItem.costPrice = product.costPrice; // auto-fill based on known product cost
            }
          }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto w-11/12">
        <DialogHeader>
          <DialogTitle>
            {isEditing
              ? `Edit Purchase Order - ${formData.referenceNo}`
              : "New Purchase Entry"}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  {suppliers.map((sup) => (
                    <SelectItem key={sup.id} value={sup.id || "unknown"}>
                      {sup.shopName}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              <Label htmlFor="refNo">Reference No. (PO Number)</Label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Delivery instructions, quality checks, etc."
                rows={2}
              />
            </div>
          </div>
